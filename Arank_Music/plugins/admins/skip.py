...
from Arank_Music.utils.inline import close_markup, stream_markup
...
async def skip(cli, message: Message, _, chat_id):
    if not len(message.command) < 2:
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Arank.stop_stream(chat_id)
                except:
                    return
        except:
            try:
    ...
    if "live_" in queued:
        ...
        try:
            ...
            image = None
        try:
            await Arank.skip_stream(chat_id, link, video=status, image=image)
        except:
        ...
        try:
            ...
            image = None
        try:
            await Arank.skip_stream(chat_id, file_path, video=status, image=image)
        except:
            return await mystic.edit_text(_["call_6"])
        button = stream_markup(_, chat_id)
        img = await get_thumb(videoid)
        ...
        try:
            await Arank.skip_stream(chat_id, videoid, video=status)
        except:
            return await message.reply_text(_["call_6"])
        button = stream_markup(_, chat_id)
        run = await message.reply_photo(
        ...
        if videoid == "telegram":
            ...
            try:
                ...
                image = None
        try:
            await Arank.skip_stream(chat_id, queued, video=status, image=image)
        except:
            return await message.reply_text(_["call_6"])
        if videoid == "telegram":
            button = stream_markup(_, chat_id)
